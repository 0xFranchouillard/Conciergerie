...
class Client
{
    ...
    function create()
    {
        ...
        $query = "INSERT INTO " . $this->table_name . " VALUES('$this->clientID','$this->agency','$this->lastName','$this->firstName','$this->email','$this->password','$this->city','$this->address','$this->phoneNumber','$this->hash')";
        // prepare w
        echo $query;
        $stmt = $this->conn->prepare($query);
        // execute query
        if ($stmt->execute()) {
            return json_encode(["userID"=>"$this->clientID"],JSON_PRETTY_PRINT);
